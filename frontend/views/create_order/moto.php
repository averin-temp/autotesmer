    ...
    <main class="request_page">
        <div class="container">
                    ...
                    <div class="form_catli">
                        <ul id="moto_category">
                            <?php foreach( Order::subcategories($model->category) as $sub_id => $label ): ?>
                                <li data-category="<?= $sub_id ?>"<?= $sub_id == $model->moto_category ? " class=\"active\"" : '' ?>>
                                    <i></i> <?= $label ?>
                                </li>
                            <?php endforeach; ?>
                        </ul>
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <div class="f_group">
                        <input type="text" placeholder="От" id="engine_volume_from" value="<?= $model->engine_volume_from ?>">
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="f_group">
                        <input type="text" placeholder="До" id="engine_volume_to" value="<?= $model->engine_volume_to ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="f_group">
                        <span class="f_group_labl">Тип двигателя</span>
                    </div>
                </div>
                <div class="col-md-10">
                    <div class="form_catli">
            ...
            <div class="row">
                <div class="col-md-12 text-center">
                    <form id="order_form" action="<?= Url::to(['/orders/save']) ?>" method="post">
                        ...
                        <input type="hidden" name="moto_category" value="">
...
$('#send').click(function(){
    ...
    form.find('input[name=moto_category]').val($('#moto_category li.active').attr('data-category'));
    form.find('input[name=comment]').val($('#comment').val() );
    form.find('input[name=engine_volume_from]').val($('#engine_volume_from').val() );
    form.find('input[name=engine_volume_to]').val($('#engine_volume_to').val() );
